      ...
      content: [
        {
          type: "text",
          text: `**${idea.title}**\n\n${idea.description}\n\nUrgency: ${idea.urgency
            }/5 | ${idea.archived ? "Archived" : "Active"} | ${idea.notes?.length || 0
            } notes`,
        },
      ],
      structuredContent: {
  ...
  async ({ idea_id, title, description, urgency, archived, notes }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Updated **${idea.title}**${changes.length > 0 ? ` (${changes.join(", ")})` : ""
            }`,
        },
      ],
      structuredContent: {
...
server.registerResource(
  "idea",
  new ResourceTemplate("ideate://ideas/{idea_id}", { list: undefined }),
  {
...
app
  .listen(port, () => {
    console.log(
      `Ideate MCP Server running on http://localhost:${port}/mcp`
    );
    console.log(`Connecting to Ideate API at: ${IDEATE_API_BASE}`);
  })
  .on("error", (error) => {
