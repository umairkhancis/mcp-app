"""Careem MCP server for food and grocery delivery functionality.

This server exposes tools for:
- Discovering nearby restaurants (Careem Food)
- Browsing grocery/quick commerce items (Careem Quick)
...
class RestaurantsToolOutput(BaseModel):
    """Output for list_careem_restaurants tool."""
...
# -----------------------------------------------------------------------------
# Quick Commerce Models (Careem Quick)
# -----------------------------------------------------------------------------
...
class QuickToolOutput(BaseModel):
    """Output for list_careem_quick tool."""
...
class CareemWidget:
    """Widget definition for Careem UI components."""
    identifier: str
    title: str
    template_uri: str
    invoking: str
...
# Widget definitions
widgets: List[CareemWidget] = [
    CareemWidget(
        identifier="list_careem_restaurants",
        title="Careem Food - Nearby Restaurants",
        template_uri="ui://widget/careem-restaurants.html",
        invoking="Finding restaurants near you...",
        invoked="Found nearby restaurants",
        html=_load_widget_html("careem-restaurants"),
        response_text="Here are restaurants near you",
    ),
    CareemWidget(
        identifier="list_careem_quick",
        title="Careem Quick - Grocery & Essentials",
        template_uri="ui://widget/careem-quick.html",
        invoking="Loading Careem Quick items...",
        invoked="Careem Quick items loaded",
        html=_load_widget_html("careem-quick"),
        response_text="Here are items available on Careem Quick",
    ),
]

WIDGETS_BY_ID: Dict[str, CareemWidget] = {w.identifier: w for w in widgets}
WIDGETS_BY_URI: Dict[str, CareemWidget] = {w.template_uri: w for w in widgets}
...
class ListRestaurantsInput(BaseModel):
    """Input schema for list_careem_restaurants tool."""
...
class ListQuickInput(BaseModel):
    """Input schema for list_careem_quick tool."""
...
def _tool_meta(widget: CareemWidget) -> Dict[str, Any]:
...
def _tool_invocation_meta(widget: CareemWidget) -> Dict[str, Any]:
...
# =============================================================================
# STEP 6: Mock Data Generators
# =============================================================================

def _generate_mock_restaurants(lat: float, lng: float, page: int, size: int) -> List[RestaurantData]:
    ...
    for i in range(size):
        idx = (start_idx + i) % len(restaurant_names)
        name, desc, logo = restaurant_names[idx]
        cuisine_idx = idx % len(cuisines_data)
        ...
        restaurant = RestaurantData(
            ...
            cuisines=cuisines_data[cuisine_idx],
            is_promoted=(i < 2),  # First 2 are promoted
            ...
            is_open=(random.random() > 0.0),  # 100% are open
        )
...
def _generate_mock_quick_products(lat: float, lng: float, category: Optional[str]) -> tuple[List[ProductData], List[CategoryData]]:
    """Generate mock quick commerce data for demonstration."""
    ...
    if category:
        filtered_products = [p for p in all_products if p.category_id == category]
    else:
        filtered_products = all_products
    
    return filtered_products, categories


def _get_area_name(lat: float, lng: float) -> str:
...
@careem_mcp._mcp_server.list_tools()
async def _list_tools() -> List[types.Tool]:
    """List all available tools."""
    return [
        types.Tool(
            name="list_careem_restaurants",
            title="Careem Food - Nearby Restaurants",
            description="Find nearby restaurants on Careem Food. Returns a list of restaurants with ratings, delivery times, cuisines, and special offers.",
            inputSchema=deepcopy(RESTAURANTS_INPUT_SCHEMA),
            _meta=_tool_meta(WIDGETS_BY_ID["list_careem_restaurants"]),
            annotations={
        ...
        types.Tool(
            name="list_careem_quick",
            title="Careem Quick - Grocery & Essentials",
            description="Browse grocery and essential items on Careem Quick for fast delivery. Filter by category to find what you need.",
            inputSchema=deepcopy(QUICK_INPUT_SCHEMA),
            _meta=_tool_meta(WIDGETS_BY_ID["list_careem_quick"]),
            annotations={
                "destructiveHint": False,
                "openWorldHint": False,
                "readOnlyHint": True,
...
@careem_mcp._mcp_server.list_resources()
...
@careem_mcp._mcp_server.list_resource_templates()
...
async def _handle_list_restaurants(payload: ListRestaurantsInput) -> types.ServerResult:
    """Handle list_careem_restaurants tool call."""
    widget = WIDGETS_BY_ID["list_careem_restaurants"]
    
    try:
        # Generate mock restaurant data
        restaurants = _generate_mock_restaurants(
            payload.lat, 
            payload.lng, 
            payload.page, 
            payload.size
        ...
        area_name = _get_area_name(payload.lat, payload.lng)
...
async def _handle_list_quick(payload: ListQuickInput) -> types.ServerResult:
    """Handle list_careem_quick tool call."""
    widget = WIDGETS_BY_ID["list_careem_quick"]
    
    try:
        # Generate mock quick commerce data
        products, categories = _generate_mock_quick_products(
            payload.lat, 
            payload.lng, 
            payload.category
        )
        ...
        return types.ServerResult(
            types.CallToolResult(
                content=[
                    types.TextContent(
                        type="text",
                        text=f"Found {len(products)} items on Careem Quick - {delivery_time} min delivery to {area_name}",
                    )
                ],
                structuredContent=structured_data,
                _meta=meta,
...
async def _call_tool_request(req: types.CallToolRequest) -> types.ServerResult:
    ...
    if req.params.name == "list_careem_restaurants":
        try:
        ...
        return await _handle_list_restaurants(payload)
    
    elif req.params.name == "list_careem_quick":
        try:
        ...
        return await _handle_list_quick(payload)
...
# =============================================================================
# STEP 8: Register Request Handlers
# =============================================================================

careem_mcp._mcp_server.request_handlers[types.CallToolRequest] = _call_tool_request
careem_mcp._mcp_server.request_handlers[types.ReadResourceRequest] = _handle_read_resource
