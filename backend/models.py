"""Domain models and persistence layer for Ideate.
...
class Idea:
    def __init__(self, title: str, description: str, urgency: UrgencyLevel = UrgencyLevel.MEDIUM):
        ...
        # persisted as JSON string in the DB
...
class IdeaStore:
    ...
    def __init__(self, db_path: str = "ideate.db"):
    ...
    def _row_to_idea(self, row) -> Idea:
        ...
        try:
            ...
            if isinstance(parsed, list):
                ...
                txt = parsed.strip()
                idea.notes = [
                    {'text': txt, 'timestamp': datetime.now().isoformat()}] if txt else []
            else:
                idea.notes = []
        except Exception:
