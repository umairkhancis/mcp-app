#!/usr/bin/env python3
"""
Simple test script to demonstrate the Ideate API functionality.
...
BASE_URL = os.environ.get(
    "IDEATE_BASE_URL", f"http://localhost:{os.environ.get('PORT', '5055')}")


def test_api():
    print("Testing Ideate API...")
    print("=" * 50)
    ...
    for idea in ideas:
        notes_preview = idea['notes'][:50] + \
            "..." if len(idea['notes']) > 50 else idea['notes']
        print(
            f"  - {idea['title']} (Urgency: {idea['urgency']}) Notes: {notes_preview}")
    print()
