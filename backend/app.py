...
# ---------------------------------------------------------------------------
# Configuration (make port configurable to avoid conflicts e.g. AirPlay on 5000)
# ---------------------------------------------------------------------------
# default moved off 5000 to avoid macOS AirPlay conflicts
PORT = int(os.environ.get("PORT", "5055"))
HOST = os.environ.get("HOST", "0.0.0.0")    # bind on all interfaces by default

# Initialize Swagger (host must reflect the runtime port)
swagger = Swagger(app, template={
    "swagger": "2.0",
    "info": {
        "title": "Ideate API",
        "description": "A simple API for managing ideas and tasks",
        "version": "1.0.0"
    },
...
# Database path (default file name in project root)
DB_PATH = os.environ.get("IDEATE_DB", "ideate.db")
...
class IdeaListResource(Resource):
    def get(self):
        ...
        include_archived = request.args.get(
            'includeArchived', 'false').lower() == 'true'
        archived_only = request.args.get(
            'archivedOnly', 'false').lower() == 'true'
...
if __name__ == '__main__':
    ...
    if idea_store.is_empty():
        idea_store.create_idea(
            "Learn Python", "Start with basic syntax and data structures", 4)
        idea_store.create_idea(
